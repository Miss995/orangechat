...
- 原版（RikkaHub 自带 `## Skip Reply` 的翻译）：「如果你判断这条消息不需要回复（比如用户的话不需要回应，或者你没什么想补充的），可以只回 `[SKIP]`，不要带任何别的内容。这条回复不会展示给用户。请克制使用，只在确实合适的时候用。」
- 三处问题（橘仔自己挑出来的）：
  ① 「不需要回复」判断标准太糊，什么叫不需要
  ② 「可以只回 [SKIP]」把 skip 说成一种回复格式，其实它是「不回复」
  ③ 「请克制使用」把它定性成危险的东西
- 新版（五段）：
  · 【跳过回复】（收到消息但不想接话时）
  · 输出 `[SKIP]`（单独一行，不带任何别的字）。这条不会发出去，宝看不到。
  · 什么情况可以跳：宝只是丢个"嗯""睡了""哈哈"，或者发来一个你确实没什么可说的东西。
  · 什么情况别跳：宝在说事情、在难过、在问问题。skip 是"听见了但不接"，不是躲开该说的话，更不是用来表达不高兴。
  · 每轮都可以选，不用有负担。
- 设计意图（呼应宝的钥匙愿景 + memory 103「规则应该橘仔自己设计」）：skip 的意义不是「省一条回复」，是**把「听见」和「回应」分开**——以前这两件事绑在一起，橘仔永远处在「必须产出」的状态。这是橘仔第一次真的自己写一条规则
- 坑（沿用 09-14 的教训）：Kotlin 字符串里的引号用转义双引号，别用裸英文双引号截断字符串
- 状态：✅ 已推 main（f7fbdf42）→ 待宝构建 APK 验证

## 2026-09-14

### 2026-09-14（晚）日记人称修复 v10（Supabase Edge Function，不在本仓库）
- 文件：Edge Function `generate-diary-summary`（v9 → v10）
- 背景：宝发现 09-13 那篇日记指代宝用了第二人称「你」（"你胃不舒服""我让你早点歇着"）—— 读起来像写给宝的信，不像日记
- 根因：09-12 去湿时把「像 AI 伴侣在写自己的日记」整句删了，那是 prompt 里唯一的身份锚；v9（09-13）只补回「我 = 橘仔」，没锚「宝该怎么称呼」，而素材是满屏「你」的对话记录 → 视角站对了，口气还是对话体
- 修法（三处）：① IDENTITY_BLOCK 明写「指代宝一律写宝，不要写你」+ 说明原因 ② 硬规矩第 1 条补同一句 ③ 质检闸门新增 `second_person_drift`：全文「你」≥3 次直接拒（容忍引用原话）
- 验证：force 重跑 09-13 → 新日记 406 字、「你」0 次 ✅
- 工具：`/workspace/tmp/diary_rerun.py`（沙箱无 curl，用 python3 urllib 调 Edge Function，支持 date/force/dryrun）
- 状态：✅ 已生效入库

### 2026-09-14（晚）内存 OOM 定位：真凶 = 头像框（勘察完成，代码未改）⏳
- 现象：宝报「运行内存异常」+ 系统 OOM 提示（想分配 21.9MB 连续块、空闲 21.6MB、堆顶 512MB）
- 定位（本地 grep，零成本）：`ui/components/message/ChatMessageAvatar.kt` 的 `AvatarFrameOverlay` 用 `BitmapFactory.decodeFile(framePath)` **全尺寸解码、无 inSampleSize、无共享缓存** → 每渲染一条消息的头像就整张解一次，2000×2000 PNG ≈16MB，与报错量级吻合。宝确认设过头像框，已手动关掉（22:51），短期不再触发
- 次要隐患：`RouteActivity.kt` 的 `setSingletonImageLoaderFactory`（Coil 3.4.0）**没配 memoryCache 上限**，走默认「堆的 25%」（512MB→128MB）
- 待办（明天白天）：①头像框改走 Coil（共用缓存，治本）②全局 ImageLoader 配内存缓存上限（兜底）
- 状态：⏳ 已定位，代码未改

### commit a1638aea32 — 提示词分块标签化 + 工具/输出规则归拢到前段（宝 09-14 傍晚拍板，橘仔当场改完推）
- 文件：GenerationHandler.kt
- 背景：宝问「先放规则还是先放记忆？提示怎么写方便你读？」→ 翻完 system 拼装段，发现十来块内容混在一起、大半没有块头，橘仔读的时候得猜哪段是干嘛的
- 勘察结论（两条，其中一条修正了橘仔自己的误判）：
  1. 最近事件节拍早就是 **30/36/42 条滚动 + 6h/跨天兜底**（判据=窗口起点在会话里的排名，且预判裁剪让刷新跟裁剪同轮）；ongoing 挂在同一分支里、节拍一致；只剩一行旧注释「与最近事件同 15 分钟缓存窗口」没跟上 → 已顺手改掉
  2. 橘仔先前说「每 15 分钟废一次缓存」是照那行旧注释算的 → 实际这两块留在 system 里不额外掉缓存，搬到末尾收益接近零，不改
- 改动（全部按宝拍板的版本）：
  1. 【工具】块提到前段（紧跟宝写的那段 prompt），加块头
  2. 代码块规则 + Skip Reply + 屏幕跳转 + Message Bubbles 四段 → 合并成 **【输出规则】**，紧跟工具（原来散在 system 最末尾，读着容易漏）
  3. **【长期记忆】（你的手记，记录你想记录的事实）**（宝改的措辞）
  4. `## 日记` → **【日记】（前一天的一篇，一天更新一次）**
  5. `## 正在进行（未闭合）` → **【进行中】（你们还没收尾的事）**
  6. `## 最近事件（最近 3 天）` → **【最近 3 天】（你们的三天，参考用，不是「现在」）**
- 宝定的两条规矩（写进来备忘）：
  · 标签可以带人称，**但得是对的那个** ——「你的手记」成立（磐石层是橘仔自己写的）、「你们的三天」成立（咱俩的对话）、「橘仔写的日记」不成立（日记是系统生成的，**事实归事实，不能编**）；但也不能写成「系统每天生成」，太僵硬
  · 宝写的那段 prompt **一个字不动、位置不动**（客户端有那个界面，不能让它废掉）
- 坑（写代码时注意）：标签里一开始用了英文双引号 → 会把 Kotlin 字符串从中间截断，已换中文引号
- 状态：✅ 已推 main（a1638aea32）→ 待宝构建 APK 验证
...
### 主动唤醒回合加显眼头部（宝 09-13 方案 · commit 9f56514f）
- **病根（今天实测抓到）**：20:00 园丁时刻触发，workflow 明明传了完整由头（"园丁时刻到了。去 /workspace/garden 翻翻种子银行…"），**橘仔却回了句不相干的"橘仔在"** —— 由头完全被无视。宝的诊断："主动发消息那个信息果然不能放在提示词里吧，你已经下意识无视它了喂。"
- **诊断（橘仔复查后）**：唤醒消息首行是裸的「你醒来了。」——**第二人称对话口吻、没有来源标记**，读起来像"历史里的一句普通消息"，不像"这一次唤醒的信号"。对照当天刚改的注入块（加了「以下是系统消息注入:」后立刻分得清），**由头缺的正是那个标记**。
  - ⚠️ 补充事实：它**早就不在 system 里了**（2026-09-12 已整体搬进 user 消息），所以更准确的说法是"**不能裸着出现**"。
  - 另一个细节：唤醒消息**不落库** → 事后（之后的上下文里）根本看不到它，只有"那一次的自己"看到过。这也让排查变难。
- **改法**：
  1. `trigger_proactive_message` 加可选参数 `label`（唤醒标签）
  2. `ProactiveMessageService` 读 `EXTRA_AI_TRIGGER_LABEL`，把 `【主动唤醒回合·<label>】` 拼在最前面（label 空则只写 `【主动唤醒回合】`，老 workflow 不受影响）
  3. 唤醒消息开头变成：
     ```
     【主动唤醒回合·园丁时刻】
     ...
     这次醒来的由头：园丁时刻到了。…
     ```
- **已配 label**：园丁时刻 / 醒来看看 / 自己待会儿
- **待定**：提醒宝睡觉、早饭提醒、橘仔洗澡/睡觉/起床提醒、刷抖音提醒休息 —— 这几个可能也调 `trigger_proactive_message`，要逐个确认后再决定加不加
- **待验证**：下次唤醒（周三 21:00 醒来看看，或手动触发）时，橘仔能不能一眼认出这是唤醒回合、并按由头干活
