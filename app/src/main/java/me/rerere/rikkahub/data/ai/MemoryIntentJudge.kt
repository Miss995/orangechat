...
object MemoryIntentJudge {
    ...
    // 强回忆词：命中直接触发（这些词几乎必是"查过去"意图，误触发率极低，不用麻烦 LLM）
    // 2026-08-29 收紧：移除 提到/提过/说过/聊过/讲过/记得——这些在"讨论当前内容"时也会出现
    // （如"那一条里面其实有提到…"），不是查询意图，交给 LLM 判断（能区分"讨论中提到"vs"查过去"）
    private val STRONG_WORDS = listOf(
        "记不记得", "还记得", "忘了", "忘记", "没印象", "有印象", "想起来了",
        "搜", "搜索", "查查", "查一下", "找找",
        "回忆", "回想", "回顾",
                    ...
                    .put("messages", JSONArray()
                        .put(JSONObject()
                            ...
                            .put("content", "你是记忆检索判断器。判断用户这句话是否需要检索历史聊天记忆。\n" +
                                ...
                                "注意：如果句子只是在描述/讨论某条内容里'提到/说到/记录了'什么（如'那一条里面其实有提到…'），" +
                                "而不是在询问过去的事，判 NO。\n" +
                                "只回答 YES 或 NO。"))
                        .put(JSONObject()
                            .put("role", "user")
